...
class ShapePattern():
    ...
    def createShapeArray(self):
        ...
        # --- 1) Create the hex profile, driven by the *user* sheet
        ...
        # --- 2) Extrude the hex, driven by the *user* sheet
        ...
        # --- 3) Make two ortho arrays.
        # Seed with neutral vectors/counts; expressions will take over afterward.
        row1Array = Draft.make_ortho_array(
            extrudedHexagonObject,
            v_x=App.Vector(1, 0, 0),
            v_y=App.Vector(0, 1, 0),
            n_x=1, n_y=1, use_link=False)
        ...
        # --- 4) Position using the target face’s bounds
        bbox = self.offset2D.Shape.BoundBox
        selMidPoint   = bbox.Center
        offset2DZPos  = bbox.ZMin

        # Set initial placements; spreadsheet expressions will refine these next.
        row1Array.Placement.Base.z = offset2DZPos  # type: ignore
        row2Array.Placement = App.Placement(       # type: ignore
            App.Vector(0, 0, offset2DZPos),
            App.Rotation(0, 0, 0),
            App.Vector(0, 0, 0))

        # --- 5) Drive arrays by the *auto-generated* sheet
        ...
        # --- 7) Fuse + center on selection
        fusedArrays = doc.addObject("Part::MultiFuse", "Fused_Arrays")
        fusedArrays.Shapes = [row1Array, row2Array]
        row1Array.Visibility = False   # type: ignore
        row2Array.Visibility = False   # type: ignore

        arraysMidPoint = fusedArrays.Shape.BoundBox.Center
        fusedArrays.Placement.Base = App.Vector(
            selMidPoint.x - arraysMidPoint.x,
            selMidPoint.y - arraysMidPoint.y,
            fusedArrays.Placement.Base.z)

        Gui.SendMsgToActiveView("ViewFit")

        # Hide the source profile after things are stable
        honeycombHexagon.ViewObject.Visibility = False

        self.fusedArrays = fusedArrays
        return fusedArrays
