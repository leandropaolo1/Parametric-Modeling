...
class SpreadSheet:
    ...
    def computingSpreadSheet(self, offset2D):
        ...
        shp = offset2D.Shape.copy()
        shp.Placement = offset2D.getGlobalPlacement()
        bbox = shp.BoundBox
...
class ShapePattern():
    ...
    def _face_from_support(self, src_obj, subname):
        """
    ...
    def createShapeArray(self):
        ...
        try:
            ...
            # ✂ removed: hex_extrude.setExpression('Placement.Base.x', ...)
            # ✂ removed: hex_extrude.setExpression('Placement.Base.y', ...)

            # 3) Two ortho arrays as links
            ...
            # ✂ removed: ZMin seating for row1/row2
            # ✂ removed: row2 placement x/y expressions

            # Drive intervals & counts from the auto sheet (keep these!)
        ...
        # keep some handles for clearing expressions later
        self._hex_extrude = hex_extrude
        self._arrays = (row1, row2)
        return compound
        

    def createPart(self, part_name="HoneycombPart"):
        doc = App.ActiveDocument
        part = doc.getObject(part_name)
        if not part or getattr(part, "TypeId", "") != "App::Part":
            part = doc.addObject("App::Part", part_name)

        # Ensure the compound is inside the Part
        if self.fusedArrays not in getattr(part, "Group", []):
            part.addObject(self.fusedArrays)
...
class Plane():
    ...
    def createShapeBinder(self):
        ...
        sel = sel_ex[0]
        src_obj = sel.Object
        subname = sel.SubElementNames[0]  # e.g. 'Face1'

        # --- walk up the parent chain to find the owning PartDesign::Body
        def _find_owning_body(o):
            visited = set()
            queue = list(getattr(o, "InList", []))  # direct parents first
            while queue:
                p = queue.pop(0)
                if p in visited:
                    continue
        ...
        body = _find_owning_body(src_obj)
        self.body = body  # may be None if selection is outside any Body

        # --- create the SubShapeBinder in the owning Body (or at root if no Body)
        if body:
...
def main():
    ...
    extrusion_a = plane.extrude(offset2D_a, spreadsheet=userSpreadsheet)   # creates object named "Extruded"
