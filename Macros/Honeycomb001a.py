...
class SpreadSheet:
    def __init__(self):
        ...
        self.set        = None           # will point to sheet.set
        self.sheet      = None           # Spreadsheet object
...
class ShapePattern():
    ...
    def createShapeArray(
        ...
        row2Array.setExpression('Placement.Base.x', f'{autoGeneratedLabel}.array2XPos')
        row2Array.setExpression('Placement.Base.y', f'{autoGeneratedLabel}.array2YPos')
        for arr in (row1Array, row2Array):
            arr.setExpression('IntervalX.x', f'{autoGeneratedLabel}.xInterval')
            arr.setExpression('IntervalY.y', f'{autoGeneratedLabel}.yInterval')
            arr.setExpression('NumberX',     f'{autoGeneratedLabel}.countX')
            arr.setExpression('NumberY',     f'{autoGeneratedLabel}.countY')
...
def main():
    ...
    # 5) Intervals + initial counts (used for array creation)
    xInterval = 2 * math.sin(math.radians(60)) * (hexRadius * 2 + (hexSeparation - 0.267949 * hexRadius))
    yInterval = 2 * hexRadius + (hexSeparation - 0.267949 * hexRadius)

    bbox = offset2D.Shape.BoundBox
    maxOffsetLength = bbox.XMax - bbox.XMin
    maxOffsetWidth  = bbox.YMax - bbox.YMin
    selMidPoint     = bbox.Center
    countY = int(maxOffsetLength / yInterval)
    countX = int(maxOffsetWidth  / xInterval)
    offset2DZPos = bbox.ZMin
    ...
    # 7) Re-bind arrays' Z tweak to the sheet
    for arr_name in ['Array', 'Array001']:
        arr = App.ActiveDocument.getObject(arr_name)
        if arr:
            arr.setExpression('Placement.Base.z', f'{autoGeneratedLabel}.tweakZ')

    # 8) Grouping, etc.
    createFolder(
        container_name="Honeycomb",
        object_names=(userSheetLabel, "Offset2D", "Extruded", "Fused_Arrays"),
        body_name="Body",
        binder_name="Binder",
        use_part=False,
        include_binder_link=True
    )
